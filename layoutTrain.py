...
from transformers import BertModel, BertTokenizer, AutoProcessor, AutoModel
import numpy as np
import os
import json
...
LAYOUTLMV3_MODEL_PATH = "./local_layoutlmv3_model"
...
def extract_layout_features(file_path, page_num=0):
    """
    Extract layout features from a PDF document
    ...
    Returns:
        tuple: (bounding_boxes, words) for the page layout
    """
    bounding_boxes = []
    words = []
    
    # Only extract layout from PDFs
    if file_path.lower().endswith('.pdf'):
        try:
            doc = fitz.open(file_path)
            ...
            for word_info in words_info:
                ...
                word = word_info[4]
                bbox = word_info[0:4]  # x0, y0, x1, y1
                
                words.append(word)
                bounding_boxes.append(bbox)
                
            return bounding_boxes, words
            
        except Exception as e:
            print(f"Error extracting layout from PDF {file_path}: {str(e)}")
    
    # Return empty lists for non-PDF files
    return [], []
...
def load_cv_data_from_folder_structure(base_dir):
    ...
    Returns:
        list: List of (cv_path, job_name, status, label, job_description_file) tuples
    """
    cv_data = []
...
class CVJobDataset(Dataset):
    ...
    Extracts:
    1. Layout features (from PDF using LayoutLMv3)
    2. Text features (CV text)
    3. Format features (additional PDF features)
    4. Job information (one hot encoded job name)
    """
    def __init__(self, cv_data, layoutlm_processor, bert_tokenizer, max_length=256, extract_format_features=True):
        """
    ...
    def __getitem__(self, idx):
        ...
        # Check cache for layout features
        if cv_path in self.layout_features_cache:
            bboxes, words = self.layout_features_cache[cv_path]
        else:
            bboxes, words = extract_layout_features(cv_path)
            self.layout_features_cache[cv_path] = (bboxes, words)
        
        # Process for LayoutLMv3
        # If no words were extracted, provide empty lists to avoid errors
        if not words:
            words = [""]
            bboxes = [[0, 0, 1, 1]]  # Dummy bbox
        
        # Create LayoutLMv3 encoding
        try:
            layoutlm_encoding = self.layoutlm_processor(
                image,
                words,
                boxes=bboxes,
                truncation=True,
            ...
            layoutlm_input_ids = layoutlm_encoding["input_ids"].squeeze()
            layoutlm_attention_mask = layoutlm_encoding["attention_mask"].squeeze()
            layoutlm_token_type_ids = layoutlm_encoding["token_type_ids"].squeeze()
            layoutlm_bbox = layoutlm_encoding["bbox"].squeeze()
            layoutlm_pixel_values = layoutlm_encoding["pixel_values"].squeeze()
            
        except Exception as e:
            print(f"Error processing layout for {cv_path}: {str(e)}")
            # Provide dummy tensors as fallback
            shape = (self.max_length,)
            layoutlm_input_ids = torch.zeros(shape, dtype=torch.long)
            layoutlm_attention_mask = torch.zeros(shape, dtype=torch.long)
            layoutlm_token_type_ids = torch.zeros(shape, dtype=torch.long)
            layoutlm_bbox = torch.zeros((self.max_length, 4), dtype=torch.long)
            layoutlm_pixel_values = torch.zeros((3, 224, 224), dtype=torch.float)
        ...
        return {
            ...
            # LayoutLMv3 inputs
            "layoutlm_input_ids": layoutlm_input_ids,
            "layoutlm_attention_mask": layoutlm_attention_mask,
            "layoutlm_token_type_ids": layoutlm_token_type_ids,
            "layoutlm_bbox": layoutlm_bbox,
            "layoutlm_pixel_values": layoutlm_pixel_values,
            # Additional features
...
class MultimodalCVClassifier(nn.Module):
    ...
    Architecture:
    1. Layout Analysis Branch:
       - LayoutLMv3 for document layout and appearance features
       - FC layers for layout score
    ...
    """
    def __init__(self, layoutlm_model_name, bert_model_name, num_classes, num_jobs, dropout_rate=0.3):
        super(MultimodalCVClassifier, self).__init__()
        
        # Load pre-trained LayoutLMv3 model for layout analysis
        self.layoutlm = AutoModel.from_pretrained(layoutlm_model_name)
        
        # Load pre-trained BERT model for semantic analysis
        self.bert = BertModel.from_pretrained(bert_model_name)
        
        # Get feature dimensions
        self.layoutlm_dim = self.layoutlm.config.hidden_size
        self.bert_dim = self.bert.config.hidden_size
        self.format_dim = 4  # Number of format features
        self.job_dim = num_jobs  # Number of unique jobs
        
        # Layout analysis branch (document appearance and layout)
        self.layout_branch = nn.Sequential(
            nn.Linear(self.layoutlm_dim, 512),
            nn.ReLU(),
        ...
        # Layout score prediction
        self.layout_score = nn.Linear(256, 1)
        ...
        self.combined_dim = 256 + 256 + 64 + 64  # layout + semantic + format + job
    ...
    def forward(self, layoutlm_input_ids, layoutlm_attention_mask, layoutlm_token_type_ids, 
                layoutlm_bbox, layoutlm_pixel_values, input_ids, attention_mask, token_type_ids, 
                format_features=None, job_one_hot=None):
        """
        Forward pass through the multimodal network with attention fusion
        
        Args:
            layoutlm_input_ids: Token IDs for LayoutLMv3
            layoutlm_attention_mask: Attention mask for LayoutLMv3
            layoutlm_token_type_ids: Token type IDs for LayoutLMv3
            layoutlm_bbox: Bounding box information for LayoutLMv3
            layoutlm_pixel_values: Image pixels for LayoutLMv3
        ...
        Returns:
            dict: Dictionary with logits, layout score, semantic score, and attention weights
        """
        # Process format features (default to zeros if not provided)
        if format_features is None:
            format_features = torch.zeros(layoutlm_input_ids.size(0), self.format_dim, 
                                         device=layoutlm_input_ids.device)
        
        # Process job features (default to zeros if not provided)
        if job_one_hot is None:
            job_one_hot = torch.zeros(layoutlm_input_ids.size(0), self.job_dim,
                                     device=layoutlm_input_ids.device)
        
        # Layout branch: Extract layout and visual features with LayoutLMv3
        layoutlm_outputs = self.layoutlm(
            input_ids=layoutlm_input_ids,
            attention_mask=layoutlm_attention_mask,
            token_type_ids=layoutlm_token_type_ids,
            bbox=layoutlm_bbox,
            pixel_values=layoutlm_pixel_values
        )
        layoutlm_features = layoutlm_outputs.pooler_output  # [batch_size, layoutlm_dim]
        layout_features = self.layout_branch(layoutlm_features)  # [batch_size, 256]
        
        # Calculate layout score
        layout_score_val = self.layout_score(layout_features)  # [batch_size, 1]
        ...
        combined_features = torch.cat(
            (layout_features, semantic_features, format_features, job_features), dim=1
        ...
        layout_weight = attention_weights[:, 0].unsqueeze(1).expand_as(layout_features)
        ...
        weighted_layout = layout_features * layout_weight
        ...
        weighted_features = torch.cat(
            (weighted_layout, weighted_semantic, weighted_format, weighted_job), dim=1
        )
...
def train_model(model, train_dataloader, val_dataloader, optimizer, criterion, epochs, device):
    """
    Train the multimodal model with layout and semantic loss components
    
    This extended training includes:
    - Classification loss: CrossEntropy for primary task
    - Layout-semantic consistency loss: To ensure correlation between branches
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            layoutlm_input_ids = batch["layoutlm_input_ids"].to(device)
            layoutlm_attention_mask = batch["layoutlm_attention_mask"].to(device)
            layoutlm_token_type_ids = batch["layoutlm_token_type_ids"].to(device)
            layoutlm_bbox = batch["layoutlm_bbox"].to(device)
            layoutlm_pixel_values = batch["layoutlm_pixel_values"].to(device)
            ...
            outputs = model(
                layoutlm_input_ids,
                layoutlm_attention_mask,
                layoutlm_token_type_ids,
                layoutlm_bbox,
                layoutlm_pixel_values,
                input_ids, 
                attention_mask, 
                token_type_ids,
                format_features,
            ...
            loss = classification_loss + 0.2 * layout_loss + 0.2 * semantic_loss
            ...
            layout_loss_total += layout_loss.item()
        ...
        layout_loss_avg = layout_loss_total / len(train_dataloader)
        ...
        print(f"  Train Loss Components: Class={classification_loss_avg:.4f}, Layout={layout_loss_avg:.4f}, Semantic={semantic_loss_avg:.4f}")
        print(f"  Val Loss: {val_loss:.4f}, Val Accuracy: {val_accuracy:.4f}")
        print(f"  Val Loss Components: Class={val_component_losses['class']:.4f}, Layout={val_component_losses['layout']:.4f}, Semantic={val_component_losses['semantic']:.4f}")

        # Save best model
        if val_accuracy > best_val_accuracy:
            best_val_accuracy = val_accuracy
            torch.save(model.state_dict(), "best_layoutlm_model.pt")
...
def evaluate_model(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Layout and semantic consistency losses
            normalized_labels = 1.0 - (labels.float() / 4.0)  # 0=1.0, 4=0.0
            layout_loss = F.mse_loss(torch.sigmoid(layout_score), normalized_labels)
            semantic_loss = F.mse_loss(torch.sigmoid(semantic_score), normalized_labels)
    ...
    component_losses = {
        ...
        'layout': layout_loss_total / len(dataloader) if len(dataloader) > 0 else 0,
...
def predict_cv(model, cv_path, job_name, job_description_file, job_one_hot, bert_tokenizer, layoutlm_processor, device):
    """
    ...
    # Process for LayoutLMv3
    try:
        layoutlm_encoding = layoutlm_processor(
            image,
            words,
            boxes=bboxes,
            truncation=True,
        ...
        print(f"Error processing layout for prediction: {str(e)}")
        # Provide dummy tensors as fallback
        shape = (1, 256)
        layoutlm_encoding = {
            ...
            "bbox": torch.zeros((1, 256, 4), dtype=torch.long),
            "pixel_values": torch.zeros((1, 3, 224, 224), dtype=torch.float)
        }
    ...
    layoutlm_input_ids = layoutlm_encoding["input_ids"].to(device)
    layoutlm_attention_mask = layoutlm_encoding["attention_mask"].to(device)
    layoutlm_token_type_ids = layoutlm_encoding["token_type_ids"].to(device)
    layoutlm_bbox = layoutlm_encoding["bbox"].to(device)
    layoutlm_pixel_values = layoutlm_encoding["pixel_values"].to(device)
    ...
    with torch.no_grad():
        ...
        logits = outputs['logits']
        layout_score = outputs['layout_score'].item()
    ...
    result = {
        ...
        "layout_score": sigmoid(layout_score),  # Normalized to [0,1]
        "semantic_score": sigmoid(semantic_score),  # Normalized to [0,1]
        "attention_weights": {
            "layout": attention_weights[0],
...
def main():
    """
    Main function to train the multimodal CV classification model using LayoutLMv3
    """
    print("Initializing multimodal CV classification training with LayoutLMv3...")
    ...
    layoutlm_processor = AutoProcessor.from_pretrained(LAYOUTLMV3_MODEL_PATH)
    ...
    # Initialize model
    print("\nInitializing multimodal model with LayoutLMv3...")
    num_classes = 5  # Five possible statuses
    num_jobs = len(dataset.unique_jobs)
    model = MultimodalCVClassifier(LAYOUTLMV3_MODEL_PATH, BERT_MODEL_PATH, num_classes, num_jobs)

    # Print model architecture summary
    print("\nModel Architecture:")
    print(f"- Layout Branch: LayoutLMv3 → {model.layoutlm_dim} → 512 → 256 → 1")
    ...
    torch.save(model.state_dict(), 'final_layoutlm_model.pt')
    print("Final model saved to final_layoutlm_model.pt")
    ...
    for i, (cv_path, job_name, status, _, job_description_file) in enumerate(cv_data):
        ...
        result = predict_cv(
            ...
            bert_tokenizer, layoutlm_processor, device
        )
        ...
        print(f"   Layout Score: {result['layout_score']:.2%}")
        print(f"   Semantic Score: {result['semantic_score']:.2%}")
        print(f"   Attention Weights: Layout={result['attention_weights']['layout']:.2f}, " 
